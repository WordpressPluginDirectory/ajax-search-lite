...
class Server {
	...
	public static function getHTTPHost(): string {
		if ( isset($_SERVER['HTTP_HOST']) && $_SERVER['HTTP_HOST'] !== '' ) {
			return $_SERVER['HTTP_HOST']; // phpcs:ignore
		}

		$bloginfo = get_bloginfo('url');
		$host     = wp_parse_url($bloginfo, PHP_URL_HOST);
	...
	public static function requestUriContains( string $needle ): bool {
		// phpcs:disable
		return strpos($_SERVER['REQUEST_URI'], $needle) !== false;
	}

	public static function refererContains( string $needle ): bool {
		$referer = wp_get_referer();
		if ( ! $referer ) {
			return false;
		}

		return strpos($referer, $needle) !== false;
	}
}
