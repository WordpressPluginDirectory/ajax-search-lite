...
class Str {
	protected static ?bool $site_is_ssl = null;

	public static function getSiteIsSSL(): bool {
		if ( self::$site_is_ssl === null ) {
			if (
				strpos(home_url('/'), 'https://') !== false ||
				strpos(plugin_dir_url(__FILE__), 'https://') !== false
			) {
				self::$site_is_ssl = true;
			} else {
				self::$site_is_ssl = false;
			}
		}
		return self::$site_is_ssl;
	}
